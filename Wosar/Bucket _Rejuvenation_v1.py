...
def bucket(depth, TLP, Rate_rejuv):

    D = 0  # depth of Bucket
    steps_to_overflow=0
    NumberofRejuvenation = 0

    while D <= depth:  # check  overflow of bucket

        steps_to_overflow=steps_to_overflow+1

        if random.random() >= Rate_rejuv: #rejuvenetion
            D = 0
            NumberofRejuvenation= NumberofRejuvenation+1
            continue
    ...
    return steps_to_overflow, NumberofRejuvenation  # steps to bucket overflow
...
RejuvEnd = [] #list to save average steps of rejuvenation
...
for j in range(nruns):
    ...
    for i in range(iterations):
        ...
        l, m = bucket(depth, TLP, Rate_rejuv)  # return of function
